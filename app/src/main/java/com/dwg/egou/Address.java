...
public class Address extends Activity implements View.OnClickListener{
    ...
    private class generateOrderAnsy extends AsyncTask<List<NameValuePair>, Void, ResponseObject> {
        ...
        protected ResponseObject<String> doInBackground(List<NameValuePair>... params) {
            ...
            httpPost.setHeader("Cookie",readUtil.GET_SESSIONID(Address.this));
        ...
        protected void onPostExecute(ResponseObject responseObject) {
            ...
            if(responseObject.getState()==1){
                Toast.makeText(Address.this, "生成订单成功", Toast.LENGTH_SHORT).show();
                i.putExtra("state",1);
            }else{
                Toast.makeText(Address.this,(String)responseObject.getDatas(), Toast.LENGTH_SHORT).show();
                i.putExtra("state",0);
            }
            setResult(1,i);
            finish();
